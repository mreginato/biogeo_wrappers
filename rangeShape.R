rangeShape <- function(data, out_shape, asc_in, thresholds, buffer.extent=5, circles=TRUE, overlay=TRUE, format=".tif") {
  getwd() -> wd
  ...
  for (i in 1:length(taxa)) {
    ...
    int[int[] > 0] <- 1
    rasterToPolygons(int, fun=function(x){x==1}, dissolve=T) -> conv2
